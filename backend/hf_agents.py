...
def call_deepseek_agent(system_prompt, payload, schema, schema_name):
    ...
    full_prompt = (
        f"{system_prompt}\n\n"
        f"You must return ONLY a valid JSON object. Do not include markdown code blocks, "
        f"comments, or any extra text. The JSON must match this exact structure:\n\n"
    ...
    logger.info(f"Calling DeepSeek API for {schema_name}...")
    try:
    ...
    if "error" in response_data:
        ...
        raise RuntimeError(f"DeepSeek API error: {response_data['error']}")

    content = response_data["choices"][0]["message"]["content"]
    logger.info(f"DeepSeek response received for {schema_name}, length: {len(content)}")

    try:
        parsed = extract_json_object(content)
        logger.info(f"Successfully parsed JSON for {schema_name}")
        return parsed
...
def generate_ai_recommendation(founder_inputs, library, algorithm_output):
    ...
    if os.getenv("DEEPSEEK_API_KEY"):
        logger.info("Using DeepSeek for AI recommendation")
        return call_deepseek_agent(system_prompt, payload, AI_RECOMMENDATION_SCHEMA, "AIRecommendation")
    return call_structured_agent(system_prompt, payload, AI_RECOMMENDATION_SCHEMA, "AIRecommendation")
...
def generate_roadmap_report(founder_inputs, algorithm_output, ai_output):
    ...
    if os.getenv("DEEPSEEK_API_KEY"):
        logger.info("Using DeepSeek for roadmap report")
        return call_deepseek_agent(system_prompt, payload, ROADMAP_REPORT_SCHEMA, "RoadmapReport")
    return call_structured_agent(system_prompt, payload, ROADMAP_REPORT_SCHEMA, "RoadmapReport")
...
def fallback_ai_recommendation(algorithm_output):
    ...
    if mode == "marketing_ready":
        branding = []
        marketing = [summarize_engine_item(item) for item in output.get("recommendations", [])[:3]]
    else:
        branding = [summarize_engine_item(item) for item in output.get("branding_recommendations", [])[:3]]
        marketing = [summarize_engine_item(item) for item in output.get("marketing_recommendations", [])[:3]]
    return {
        "mode": mode,
...
def fallback_roadmap_report(founder_inputs, algorithm_output, ai_output):
    ...
    if mode == "branding_first":
        ...
        roadmap.append({
            ...
            "timeframe": "Days 1-3",
            "actions": [f"Complete {item['name']}" for item in top_branding] or ["Complete the top ranked branding strategy from the decision engine."],
            "success_metrics": [
        ...
        roadmap.append({
            ...
            "timeframe": "Days 4-10",
            "actions": [f"Run {item['name']}" for item in top_marketing] or ["Run the top ranked marketing experiment after branding is clearer."],
            "success_metrics": [
        ...
        roadmap.append({
            ...
            "timeframe": "This week",
            "actions": [f"Run {item['name']}" for item in top_marketing] or ["Run the top ranked marketing experiments from the decision engine."],
            "success_metrics": [
